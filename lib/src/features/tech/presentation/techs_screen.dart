...
import 'package:portfolio/src/features/tech/presentation/widgets/tech_widget.dart';

class TechsScreen extends StatelessWidget {
  final List<SkillSet> skillSet;

  const TechsScreen({super.key, required this.skillSet});
            ...
            SliverList.builder(
                ...
                itemBuilder: (context, index) {
                  return Container(
                      ...
                      child: TechWidget(
                        title: "${skillSet[index].name}",
                        level: "${skillSet[index].level}",
                        iconPath: "assets/icons/tech/symfony.png",
                        color: Color(int.parse('0xFF${skillSet[index].color}')),
                      ));
                }),
            SliverToBoxAdapter(
              child: SizedBox(
